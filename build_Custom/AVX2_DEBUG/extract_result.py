...
def main():
    ...
    for m_index,method_name in enumerate(Methods):
        test_batch_folder_path = os.path.join(base_path,method_name)
        #print(os.listdir(test_batch_folder_path))

        for np_num in os.listdir(test_batch_folder_path):
            single_bench_result_path = os.path.join(test_batch_folder_path,f"{np_num}","Result.json") 
            if os.path.isfile(single_bench_result_path):
                ...
                for item_name,value in raw_data.get("User Optimization Overheads",{}).items():
                    new_data_bundle[item_name] = value
                for item_name,value in raw_data.get("DDOT Timing Variations",{}).items():
                    new_data_bundle[item_name] = value
                
                for item_name,value in raw_data.get("Benchmark Time Summary",{}).items():
                    new_data_bundle[f"{item_name} - Execution Time (s)"] = value
                
                new_data_bundle["Result - GFLOP/s"] = raw_data.get("Final Summary",{}).get("HPCG result is VALID with a GFLOP/s rating of")

                new_data_key = f"{method_name}-{np_num}"
