...
int HPCG_Init(int* argc_p, char*** argv_p, HPCG_Params& params) {
  ...
  //Set global method variable



  int mt_index = iparams[10];
  if (mt_index < (int)OPTIM_TYPE_REF || mt_index >(int)OPTIM_TYPE_LMB) {
    g_optimization_type = OPTIM_TYPE_REF;
    printf("Invalid Methology Index:%d, using Reference method.\n", mt_index);
  }
  else {
    g_optimization_type = static_cast<OptimizationType>(mt_index);
